...
type ClientInterface interface {
	...
	DeleteKey(user passwd.User, key string) error
	AddKey(user passwd.User, key string) error
	WriteKeys(user *passwd.User, keys []authorized_keys.SSHKey) error
	CreateSSHDir(user *passwd.User) error
	ChownHomedir(user *passwd.User) error
	ChownSSH(user *passwd.User) error
}
